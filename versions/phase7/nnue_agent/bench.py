"""Nodes-per-second bench for the NNUE agent, mirroring tools/bench.py's fixed
positions exactly so the two numbers are directly comparable. No games, no
harness/subprocess overhead -- one direct search_move() call per position.
Speed work that does not move this number is not speed work.
"""
